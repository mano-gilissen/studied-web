                                ...
                                <tr>
                                    <td style="font-family: sans-serif; font-size: 14px; vertical-align: top;">
                                        ...
                                        @if(\App\Http\Traits\StudyTrait::hasLink($study))
                                            ...
                                            <p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 32px;">De gegevens van je les {{ \App\Http\Traits\StudyTrait::getSubject($study)->{\App\Http\Support\Model::$SUBJECT_NAME} }} met {{ $study->getHost->getPerson->{\App\Http\Support\Model::$PERSON_FIRST_NAME} }} zijn gewijzigd. Deze vindt nu plaats op {{ strtolower(\App\Http\Support\Format::datetime($study->start, \App\Http\Support\Format::$DATETIME_SINGLE)) }} van {{ \App\Http\Traits\StudyTrait::getTimeText($study, true) }} en de locatie is: <a href="{{ $study->{\App\Http\Support\Model::$STUDY_LINK} }}">{{ $study->{\App\Http\Support\Model::$STUDY_LINK} }}</a>. Kom op tijd en zorg ervoor dat je alles bij je hebt wat je normaal ook naar school zou meenemen.</p>

                                        @else

                                            <p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 32px;">De gegevens van je les {{ \App\Http\Traits\StudyTrait::getSubject($study)->{\App\Http\Support\Model::$SUBJECT_NAME} }} met {{ $study->getHost->getPerson->{\App\Http\Support\Model::$PERSON_FIRST_NAME} }} zijn gewijzigd. Deze vindt nu plaats op {{ strtolower(\App\Http\Support\Format::datetime($study->start, \App\Http\Support\Format::$DATETIME_SINGLE)) }} van {{ \App\Http\Traits\StudyTrait::getTimeText($study, true) }} en de locatie is: {{ $study->{\App\Http\Support\Model::$STUDY_LOCATION_TEXT} }}. Kom op tijd en zorg ervoor dat je alles bij je hebt wat je normaal ook naar school zou meenemen.</p>
