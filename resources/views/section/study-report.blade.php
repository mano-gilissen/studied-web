<div class="block-attributes block-report">
    ...
    <div class="top">
        ...
        @if(\App\Http\Traits\BaseTrait::hasManagementRights())
            ...
            <img id="flag" src="/images_app/flag-report{{ $study->report_flagged ? '-active' : '' }}.svg" onclick="report_flag({{ $study->{\App\Http\Support\Model::$BASE_KEY} }})"/>
    ...
    @foreach($study->getParticipants_User as $participant)
        ...
        <div class="report" id="report_{{ $participant->id }}">
            ...
            <div style="display:flex">
                ...
                <div id="{{ $study->getHost->getPerson->first_name }}" class="person-report" onclick="navigate('{{ route('person.view', ['slug' => $study->getHost->getPerson->slug]) }}')">
            ...
            <div class="subjects">
                ...
                @foreach($report->getReport_Subjects as $report_subject)
                    ...
                    <div class="subject">
                        ...
                        <div class="name">{{ __($report_subject->getAgreement ? \App\Http\Traits\AgreementTrait::getVakcode($report_subject->getAgreement) : $report_subject->getSubject->{ \App\Http\Support\Model::$SUBJECT_NAME}) }}</div>
